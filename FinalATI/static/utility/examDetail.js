...
function handleRemoveExam(id, subject_id) {
  window.location.href =
    "/delete_student_from_subject?id=" + id + "&subject_id=" + subject_id;
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  submitButton.addEventListener("click", async function () {
    if (fileInput.files.length > 0) {
      ...
      // receive fetch result here
      const dataFetch = await result.json();
      if (dataFetch.fetchStatus == true) {
        window.location.href = "/detail?id=" + subjectID;
      } else {
        handleInvalidImg(contentUpload);
      }
    }
  });

  function handleInvalidImg(contentUpload) {
    contentUpload.setAttribute("success-data", "true");
    const submit_rs = document.querySelector(".content__upload .submit-result");
  
    if (submit_rs) {
      submit_rs.classList.remove("hidden"); // Hiện thông báo
  
      // Ẩn thông báo sau 1 giây
      setTimeout(() => {
        submit_rs.classList.add("hidden");
      }, 1000);
    } else {
      console.log("submit_rs is not properly defined.");
    }
  }
